...
#![allow(unused_imports)] // Allowed until all tests are enabled
...
mod owning_ref {
    ...
    #[test]
    fn total_erase() {
        let a: OwningRef<'_, Vec<u8>, [u8]> = OwningRef::new(vec![]).map(|x| &x[..]);
...
mod owning_ref_mut {
    ...
    fn fmt_debug() {
        ...
        let or = or.map_mut(|x| &mut x[..5]);
        let s = format!("{:?}", or);
        assert_eq!(&s, "OwningRefMut { owner: _, reference: \"hello\" }");
    }
    ...
    #[test]
    fn total_erase() {
        let a: OwningRefMut<'_, Vec<u8>, [u8]> = OwningRefMut::new(vec![]).map_mut(|x| &mut x[..]);
